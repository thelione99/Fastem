...
// Helper per iniettare le variabili CSS e gestire lo sfondo dinamico
const ThemeInjector: React.FC = () => {
  const settings = useSettings();
  
  useEffect(() => {
    // Iniettiamo le variabili CSS per l'uso globale (es. nei bottoni)
  ...
  return (
    // Colore di sfondo base applicato qui
    <div className="fixed inset-0 z-0 pointer-events-none transition-colors duration-700" style={{ backgroundColor: settings.bgColor }}>
        <DotGrid 
            dotSize={2} 
            gap={25} 
            // Colori dinamici per la griglia
